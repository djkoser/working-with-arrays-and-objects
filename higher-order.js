...
//Code Here
let evenNumbers = mixedNumbers.filter(el=> el%2===0 ?true:false)// = mixedNumbers.filter(/* Provide Your Callback Here */)
...
//Code Here
let postTaxPrices = prices.map(el=> el*(1+0.07))// = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((agg,curr) =>agg+curr)//  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(obj=> obj.CP>200?true:false)// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals= orders.map(curr => curr.price+(curr.price*curr.tax)); // Code here
...
let bobsTotal = purchases.filter(obj=>obj.owner==="Bob"?true:false).reduce((agg,obj)=>agg+obj.price, 0)
